...
function lightbox_func($atts, $content) {
	...
	if(!empty($youtube_id))
	{
		$return_html.= '<div style="display:none;"><div id="video_'.$youtube_id.'" style="width:640px;height:385px">
...
function pp_pre_func($atts, $content) {
	...
	$return_html = '<div class="pp_accordion_close"><h3><a href="#">Get The code</a></h3>';
	$return_html.= '<div><pre>';
	$return_html.= do_shortcode($content);
	$return_html.= '</pre></div></div>';
...
function map_func($atts) {
	...
	$return_html = '<div class="frame" id="map'.$custom_id.'" style="width:'.$width.'px;height:'.$height.'px"></div>';
	$return_html.= '<script>';
...
function nivoslide_func($atts, $content) {
	...
	if(!empty($gallery_id))
	{
		...
		$image_arr = get_posts( $args );
		
		$rand_id = mt_rand();
		$return_html = '<div class="nivo_border" style="width:'.$width.'px;height:'.$height.'px;"><div id="'.$rand_id.'" class="nivoslide" style="width:'.$width.'px;height:'.$height.'px; visibility: hidden;display:none">';
		...
		if(!empty($image_arr) && is_array($image_arr))
		{
			foreach($image_arr as $image)
			{
				...
				{
				    $image_id = $image->ID;
				    $image_url[0] = $image->guid;
				}
				
				if(!empty($image))
				{
					$return_html.= '<img src="'.get_bloginfo( 'stylesheet_directory' ).'/timthumb.php?src='.strip_tags($image_url[0]).'&amp;h='.$height.'&amp;w='.$width.'&amp;zc=1" alt=""/>';
				}
			}
		}
		...
		$return_html.= "<script>jQuery(window).load(function() { jQuery('#".$rand_id."').css('visibility', 'visible'); jQuery('#".$rand_id."').nivoSlider({ pauseTime: ".intval($pauseTime*1000).", pauseOnHover: true, effect: '".$effect."', controlNav: true, captionOpacity: 1, directionNavHide: false, controlNavThumbs: true, controlNavThumbsFromRel:false, boxCols:8, boxRows:4, afterLoad: function(){ 
			jQuery('#".$rand_id."').css('display', 'block');
		} }); });</script>";
	}
	...
	if(!empty($gallery_id))
	{
		...
		if(!empty($image_arr) && is_array($image_arr))
		{
			foreach($image_arr as $image)
			{
				...
				if(!empty($image))
				{
					$return_html.= '<a href="'.$image_url[0].'">';
					$return_html.= '<img src="'.get_bloginfo( 'stylesheet_directory' ).'/timthumb.php?src='.strip_tags($image_url[0]).'&amp;h=60&amp;w=120&amp;zc=1" alt=""/>';
					$return_html.= '</a>';
				}
			}
		}
		...
		$return_html.= "<script>jQuery(window).load(function() { jQuery('#".$rand_id."').galleria({ width:".$width.", height:".$height.", transition: 'fade' }); });</script>";
	}
