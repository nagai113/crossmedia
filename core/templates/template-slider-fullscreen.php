...
#outer_container{position:relative; padding: 7px 0 0 0; width:100%; margin: 0; background: #fff;}
<?php
	}
	else
	{
...
#outer_container{position:relative; padding: 7px 0 0 0; width:100%; margin: 0; background: #000;}
<?php
	}
?>
...
#img_title{position:fixed; z-index:3; right:10px; top:10px; padding:10px; background:#fff; color:#444; font-family: Arial, serif; font-size:24px; text-transform:uppercase;display:none; }
#bg{position:fixed; z-index:1; overflow:hidden; width:100%; height:100%; margin: 0;}
#bgimg{display:none; -ms-interpolation-mode: bicubic;}
#preloader{position:relative; z-index:3; width:32px; padding:20px; top:160px; margin:auto; background:#000;}
<?php
$pp_display_slideshow_thumb = get_option('pp_display_slideshow_thumb');
?>
#thumbnails_wrapper{opacity:0;z-index:2; position:fixed; bottom:0; width:100%; background:url(<?php echo get_stylesheet_directory_uri(); ?>/js/fullscreen/empty.gif); /* stupid ie needs a background value to understand hover area */
<?php if(empty($pp_display_slideshow_thumb)) { echo 'visibility:hidden;'; } ?>
}
...
#outer_container .thumbScroller img{margin: 0 10px 0 0; }
...
<div class="thumbScroller">
	<div class="container">
		...
		?>
    	<div class="content">
        	<div><a href="<?php echo $image_url[0]; ?>"><img src="<?php echo $small_image_url; ?>" title="" alt="" class="thumb" /></a></div>
        </div>
        <?php
        	}
        ?>
...
$jthumbnailsContainerOpacity=1; //thumbnails area default opacity
...
<?php
	$pp_auto_play = get_option('pp_auto_play');
